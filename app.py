...
with col2:
    ...
    if not st.session_state.new_cart:
        ...
        # 顯示對應的支付說明 UI
        if st.session_state.pay_choice == "DuitNow 線上轉賬":
            ...
            if st.session_state.get("receipt_uploader") is not None:
                st.success("✅ 收據已成功載入！")
        ...
        elif st.session_state.pay_choice == "到店支付現金 / 拿食物時付款":
            st.info("💡 提示：請在下單後，於現場取餐/用餐時向櫃檯支付現金。")
            payment_closing_text = f"老闆，我選擇【到店支付現金】，請先幫我準備單號 {st.session_state.order_id} 的餐點，我抵達時再付款，謝謝！"

        st.write("---")
        
        # 🔥【高能修正】：按鈕永遠開啟 (disabled=False)，改用點擊後內部判斷引導
        if st.button("🚀 確認無誤，送出訂單到 WhatsApp", key="submit_order", disabled=False, use_container_width=True):
            
            # 開始後台安全檢查
            check_passed = True
            
            if st.session_state.dining_choice is None:
                st.error("❌ 錯誤：您尚未在最上方選擇「用餐方式」！")
                check_passed = False
                
            elif st.session_state.dining_choice == "外送 / 食物配送 🚗":
                addr = st.session_state.get("addr_val", "").strip()
                if not addr:
                    st.error("❌ 錯誤：請填寫「完整外送地址」，否則無法外送！")
                    check_passed = False
                    
            elif st.session_state.dining_choice == "內用 🍽️":
                table = st.session_state.get("table_val", "").strip()
                if not table:
                    st.error("❌ 錯誤：內用請填寫「桌號」！")
                    check_passed = False
            
            if st.session_state.pay_choice is None:
                st.error("❌ 錯誤：請選擇您的「付款方式」！")
                check_passed = False
            ...
            if check_passed:
                ...
                st.success("🎉 訂單已成功生成！正在為您開啟 WhatsApp 連線老闆...")
                st.rerun()
